-- Don Evaristo, etapa 1: memoria propia (conversaciones que sobreviven a la
-- pantalla y al dispositivo) y contexto en vivo del cliente para asesorar.
--
--   evaristo_conversaciones / evaristo_mensajes: historial por usuario, RLS propia.
--   evaristo_contexto(p_codigo): todo lo que Don Evaristo necesita saber del
--   usuario que llama (empresa, plan, inventario, extensión, oportunidades con
--   match, ofertas, señales, libros del experto, tickets) y, si está mirando
--   una licitación o compra ágil, su ficha resumida y el match de sus ítems.

create table if not exists public.evaristo_conversaciones (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
  cliente_id uuid,
  canal text not null default 'app',          -- app | landing | email | audio
  titulo text,
  contexto jsonb,                              -- último contexto en vivo (pantalla, código)
...
create table if not exists public.evaristo_mensajes (
  ...
  adjuntos jsonb,                              -- [{tipo:'imagen'|'audio', nombre, bytes}] sin el binario
  meta jsonb,                                  -- {modelo, ms, pantalla, codigo, tipo:'saludo'}
  creado_en timestamptz not null default now()
);
...
-- Contexto en vivo del usuario que llama. SECURITY DEFINER para poder cruzar
-- tablas con RLS distinta; siempre acotado a auth.uid() / su cliente.
