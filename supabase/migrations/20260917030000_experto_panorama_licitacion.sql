-- Experto: panorama completo de una licitación para que Don Evaristo note lo que
-- falta y lo pida. Devuelve, en una sola llamada:
--   documentos_de_otra_licitacion: bases subidas bajo este código que mencionan OTRO
--     código y no el propio (subidas por error o antecedentes del mismo organismo).
--   antecedentes: licitaciones anteriores del mismo organismo con nombre parecido
--     (3 años), con adjudicatarios, si ya tenemos sus bases y cuántos reclamos tuvieron.
--   compras_agiles_relacionadas: compras ágiles del mismo organismo en 180 días con
--     el mismo tema (posible fragmentación).
--   reclamos_organismo: reclamos de proveedores contra el organismo (12 meses).
--   matches_cliente: ítems de la licitación que calzan con el inventario del usuario.
...
revoke execute on function public.experto_panorama_licitacion(text, uuid) from public, anon;
grant execute on function public.experto_panorama_licitacion(text, uuid) to authenticated, service_role;
