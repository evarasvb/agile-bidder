...
-- Descarga: cada 2 minutos, hasta 6 licitaciones por corrida (presupuesto de 110 s).
...
select cron.schedule('licitacion-adjuntos-auto', '*/2 * * * *', $$
  select net.http_post(
    ...
    body := '{"auto":true,"limit":6}'::jsonb, timeout_milliseconds := 120000);
$$);

-- Lectura de bases (texto + resumen): cada 5 minutos, hasta 3 PDF por corrida.
...
select cron.schedule('licitacion-bases-pendientes', '1,6,11,16,21,26,31,36,41,46,51,56 * * * *', $$
  select net.http_post(
    ...
    headers := jsonb_build_object('Content-Type','application/json',
      'Authorization','Bearer ' || (select decrypted_secret from vault.decrypted_secrets where name = 'service_role_jwt_legacy')),
    body := '{"bases":true,"limit":3}'::jsonb, timeout_milliseconds := 120000);
$$);
