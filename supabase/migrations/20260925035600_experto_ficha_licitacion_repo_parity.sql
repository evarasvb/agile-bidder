-- Documenta en el repo el estado real de experto_ficha_licitacion. Codex marcó
-- que el aviso "No encontré la licitación o compra..." (abogado-consultar,
-- gatillado por !res.ficha) podría reportar un falso negativo para un código de
-- COMPRA ÁGIL válido, asumiendo que esta función solo cubre licitaciones. Se
-- verificó contra pg_get_functiondef en la base viva: la función YA hace
-- coalesce(ficha_licitacion, ficha_compra_agil) — si el código es una compra
-- ágil real, `res.ficha` sí llega poblado (con 'tipo': 'Compra Ágil'), así que
-- el aviso solo se dispara cuando el código no existe en ninguna de las dos
-- tablas, que es el comportamiento correcto. Esta función nunca había quedado
-- como archivo en el repo (mismo tipo de brecha de paridad ya encontrada antes
-- en las políticas de `vendedores`); esta migración es idempotente y solo deja
-- explícito en el repo el estado correcto, sin cambiar el comportamiento.
