-- El libro guarda tambien el mapa conceptual (consultas modo 'mapa', respuesta JSON). Aplicado como experto_libro_mapa.
...
returns jsonb language sql stable security definer set search_path = public, experto as $$
  ...
  select jsonb_build_object(
    ...
    'licitaciones_similares', (select coalesce(jsonb_agg(jsonb_build_object('codigo', t.codigo, 'titulo', t.titulo, 'fecha_cierre', t.fecha_cierre, 'monto_estimado', t.monto_estimado, 'monto_adjudicado', t.monto_adjudicado, 'adjudicatario', t.adjudicatario_nombre, 'rut', t.adjudicatario_rut, 'tipo', t.tipo_producto)), '[]'::jsonb) from public.experto_licitaciones_similares(upper(p_codigo), 12, 5) t),
