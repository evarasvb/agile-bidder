...
SET 
  tipo_proceso = CASE
    WHEN monto IS NULL OR monto = 0 THEN 'compra_agil'
    WHEN monto <= 6975100 THEN 'compra_agil'  -- <= 100 UTM
    ELSE 'licitacion'  -- > 100 UTM
  END,
  categoria = CASE
    WHEN monto IS NULL OR monto = 0 THEN 'L1'
    WHEN monto <= 6975100 THEN 'L1'  -- < 100 UTM = Compra Ágil
    WHEN monto <= 69751000 THEN 'LE'  -- 100 a 1.000 UTM = Intermedia
    WHEN monto <= 348755000 THEN 'LP'  -- 1.000 a 5.000 UTM = Mayor
