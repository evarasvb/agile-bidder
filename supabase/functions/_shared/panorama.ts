  ...
  if (docs.length) {
    out.push("DOCUMENTOS SUBIDOS BAJO ESTE CÓDIGO QUE HABLAN DE OTRA LICITACIÓN (detección automática por el código que aparece en el texto):\n" +
      docs.map((d) => `- "${d.archivo}" menciona ${d.codigo_detectado} y no ${codigo} → ${d.relacion === "antecedente_mismo_organismo"
        ? "es una licitación ANTERIOR del mismo organismo: sirve como antecedente y contexto, no como bases de esta"
        : "parece de OTRA licitación (otro organismo): probablemente se subió aquí por error; no son las bases de esta licitación"}`).join("\n"));
