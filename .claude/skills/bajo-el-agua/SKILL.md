---
name: bajo-el-agua
description: Modo Bajo el Agua. Investigación profunda de una licitación o compra ágil de Mercado Público (Chile) a partir de su ID. Deduce relaciones ocultas (historial del organismo, proveedor recurrente, convenio de suministro, compras ágiles y convenio marco del mismo producto, comisiones evaluadoras repetidas, audiencias de lobby, consultas al mercado (RF/RFI) previas, desiertas o nulas, precio promedio, noticias, resoluciones, dictámenes), estudia las bases y anexos, arma la matriz de adjudicación y la simulación, y si ya está adjudicada busca por dónde renovarla. Usar cuando se diga "bajo el agua", "mira debajo del agua", "investiga la licitación ID", "qué hay detrás de esta licitación", "deep research licitación", o cuando el usuario pegue un ID de Mercado Público (formato 1234-56-LE24, 1234-56-LR24, 1234-56-LQ24, 1234-56-L124, 1234-56-COT24).
---
...
- **Consultas al mercado (RFI)** previas del organismo o del producto (ver fase 3, RF): si la licitación nació de una, leer sus preguntas y adjuntos.
...
- **Ley del Lobby (Ley 20.730)**: la plataforma oficial es `https://www.leylobby.gob.cl`. No tiene API pública vigente (`/api/v1` responde 404); se leen sus páginas públicas:
  1. Buscar el organismo: `https://www.leylobby.gob.cl/instituciones?search=<nombre>` (tabla Servicio | Url, el código es como `MU283` para municipios, `AB001` para subsecretarías, `CM…` corporaciones municipales).
  2. Autoridades con registro del año: `https://www.leylobby.gob.cl/instituciones/<CODIGO>/audiencias/<AÑO>` (nombre, cargo, link "Ver Detalle").
  3. Audiencias de cada autoridad: `https://www.leylobby.gob.cl/instituciones/<CODIGO>/audiencias/<AÑO>/<ID>` (Fecha, Identificador, Asistentes con su calidad: sujeto pasivo, lobbista, gestor de intereses; Representados; Materia).
  Revisar los últimos 24 meses, priorizar alcalde o jefe de servicio, administrador, directores, jefes de adquisiciones o abastecimiento. Marcar como relacionadas las audiencias cuya materia o representados hablen de compras, licitaciones, convenios o del rubro, y las que tengan al proveedor dominante. En Agile Bidder esto lo hace solo la función `experto-bajo-agua` (con caché de 7 días en `experto_fuentes_cache`).
- **RF = Consulta al Mercado (RFI, Request for Information)**: es lo que el organismo publica ANTES de licitar para preguntarle al mercado precios, características y plazos. Códigos con sufijo `-RFI` (ej. `2558-3-RFI25`). Portal: `https://consulta-mercado.mercadopublico.cl` (ficha: `/detalle-consulta-mercado/<ID>`). API pública del portal: token anónimo en `GET https://servicios-prd.mercadopublico.cl/v1/auth/publico` (campo `payload.access_token`), luego con `Authorization: Bearer` en `https://servicios-consultas-prd.mercadopublico.cl`: listado `GET /v1/consulta-mercado?desde=AAAA-MM-DD HH:MM:SS&hasta=…&estado=&ordenarPor=&pagina=1&palabraClave=…&organismoComprador=…` y detalle `GET /v1/consulta-mercado/codigo/<ID>` (nombre, descripción, motivo, organismo y RUT, encargado, fechas, preguntas, reuniones, adjuntos). Qué buscar: si esta licitación nació de una RFI del mismo organismo (mismo objeto, fechas previas), quién preguntó y participó en reuniones (ya conoce las bases), y RFIs del mismo producto en otros organismos (licitaciones que vienen).
