      ...
      <div className="rounded-lg border p-3 space-y-2">
        ...
        {tareas.map((t, i) => (
          <div key={i} className="grid grid-cols-1 sm:grid-cols-[auto_1fr_auto_auto] items-start sm:items-center gap-2 text-xs border-b last:border-0 py-2">
            ...
            <input value={t.responsable ?? ''} onChange={(e) => setTarea(i, 'responsable', e.target.value)} placeholder="responsable" className="min-w-[120px] bg-transparent border-b border-dashed border-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-primary text-sm" aria-label={`Responsable de ${t.accion}`} />
            <input value={t.plazo ?? ''} onChange={(e) => setTarea(i, 'plazo', e.target.value)} placeholder="plazo (ej: 30 dic)" className="min-w-[100px] bg-transparent border-b border-dashed border-muted-foreground/40 focus:outline-none focus:ring-1 focus:ring-primary text-sm" aria-label={`Plazo de ${t.accion}`} />
          </div>
        ))}
      </div>
