                      ...
                      <div className="space-y-1">
                        ...
                        <div className="text-sm text-muted-foreground">
                          {factura.por_suscripcion
                            ...
                            : <>Fijo neto {formatCurrency(factura.fijo_monto ?? 0)} + comisión 3% del neto {formatCurrency(factura.total_comision)} + IVA {formatCurrency(((factura as any).iva_fijo ?? Math.round((factura.fijo_monto ?? 0) * 0.19)) + ((factura as any).iva_comision ?? Math.round(factura.total_comision * 0.19)))} = <b>{formatCurrency(factura.total ?? Math.round(((factura.fijo_monto ?? 0) + factura.total_comision) * 1.19))}</b></>}
