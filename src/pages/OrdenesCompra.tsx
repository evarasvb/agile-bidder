...
import { Search, FileText, Building2, User, Calendar, Package, X, Filter, RefreshCw, ArrowDownWideNarrow, Download } from "lucide-react";
...
export default function OrdenesCompra() {
  ...
  const ordenesMostradas = ordenarPorMonto
      ? [...ordenes].sort((a, b) => (b.total ?? 0) - (a.total ?? 0))
        : ordenes;
        ...
        <div className="space-y-6 p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Button
                onClick={() => setOrdenarPorMonto((v) => !v)}
            variant={ordenarPorMonto ? "default" : "outline"}
            className="gap-2"
          >
              ...
              {orden.institucion_rut && (
                ...
                {orden.proveedor_rut && (
                  ...
                  {orden.estado || 'N/A'}
                                        ...
                                        <TableCell>
                  {esTratoDirecto(orden.tipo) ? (
                                              <Badge variant="destructive" className="whitespace-nowrap">
                                                Trato Directo
                                              </Badge>
                                            ) : (
                                                                          <Badge variant="outline">{getTipoOCLabel(orden.tipo)}</Badge>
                                          )}
                                        </TableCell>
                  ...
                  {ordenDetalle && (
                                                      <Button
                                                        ...
                                                        size="sm"
                                                        className="gap-2"
                                                        onClick={() => descargarOrdenCompraPDF(ordenDetalle)}
                                                    >
                                                      <Download className="h-4 w-4" />
                                                      Exportar PDF
                                                    </Button>
                  ...
                  {orden.total ? formatCurrency(orden.total) : 'N/A'}
                              ...
                              <div>
                                ...
                                <div>
                  {esTratoDirecto(orden.tipo) ? (
                                      <Badge variant="destructive">Trato Directo</Badge>
                                    ) : (
                                                          <Badge variant="outline">{getTipoOCLabel(orden.tipo)}</Badge>
                                  )}
                                </div>
